...
async function calculateOrderAmount(items: CartItem[], response: express.Response) {
  ...
  for (let i = 0; i < items.length; i++) {
    let product:
      ...
      | (db.ProductVariant & { product: db.Product } & {
          productVariantAttributes: (db.ProductVariantAttribute & { attribute: db.Attribute })[];
        })
      | null;
    ...
    if (items[i].variant) {
      product = await prisma.productVariant.findFirst({
        where: {
          AND: [{ id: items[i].variant?.id }, { productId: items[i].productId }],
        },
        include: { product: true, productVariantAttributes: { include: { attribute: true } } },
      });
      ...
      if (product && !product.available) {
        ...
        productsNotAvailable.push({ productName: product.product.name, isVariant: true, variantName });
      }
    } else {
      ...
      //product availability validation
      if (product && !product.available) {
        productsNotAvailable.push({ productName: product.name, isVariant: false, variantName: "" });
      }
    }
    if (product) {
      sum += product.price;
      ...
      if (items[i].variant) {
        let variantName = "";
        const productVariantAttributes = items[i].variant!.productVariantAttributes;
        ...
        productsNotExist.push({ productName: items[i].productName, isVariant: true, variantName });
      } else {
        productsNotExist.push({ productName: items[i].productName, isVariant: false, variantName: "" });
      }
    }
  }
  if (productsNotExist.length) {
    throw {
      message: "Product(s) in the cart does not exist. Remove the following product(s) from your cart:",
      products: JSON.stringify(productsNotExist),
    };
  }
  if (productsNotAvailable.length) {
    throw {
      message: "Product(s) in the cart does are not available. Remove the following product(s) from your cart:",
...
router.post("/create-payment-intent", async (request: express.Request, response: express.Response) => {
  try {
    ...
    if (error.type === "StripeInvalidRequestError") {
      ...
      if (error.code === 1234) {
        return response.status(400).json({ message: error.message, productsNotExist: error.productsNotExist, code: error.code });
      }
      if (error.code === 1235) {
        return response.status(400).json({ message: error.message, productsNotAvailable: error.productsNotAvailable, code: error.code });
      }
...
router.post("/create-order", async (request: express.Request | any, response: express.Response) => {
  ...
  try {
    ...
    return response.status(200).json({ message: "success", order: order.id, cart: cart.id });
  } catch (error: any) {
